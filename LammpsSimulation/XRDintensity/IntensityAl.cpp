...
class IntensityAl : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit IntensityAl(const ActionOptions&);
  ~IntensityAl();
// active methods:
...
PLUMED_REGISTER_ACTION(IntensityAl,"INTENSITYAL")

void IntensityAl::registerKeywords( Keywords& keys ){
...
IntensityAl::IntensityAl(const ActionOptions&ao):
...
IntensityAl::~IntensityAl(){
  delete nl;
}

void IntensityAl::prepare(){
  if(nl->getStride()>0){
...
void IntensityAl::calculate()
{
  ...
  for(unsigned int m=0;m<qhist;++m){
    ...
    q=4*pi*std::sin(theta[m]*pi/180)/lamda;
      ...
      fij_Al= 6.4202*exp(-3.0387*(q/(4*pi))*(q/(4*pi)))+1.9002*exp(-0.7426*(q/(4*pi))*(q/(4*pi)))+1.5936*exp(-31.5472*(q/(4*pi))*(q/(4*pi)))+1.9646*exp(-85.0886*(q/(4*pi))*(q/(4*pi)))+1.1151;
     ...
     distance_versor = distance / distanceModulo;
      ...
      value[m]+=2*std::sin(q*distanceModulo)/(getNumberOfAtoms()*q*distanceModulo);        
       ...
       comm.Sum(sfVirial);
         ...
         structureF[m]=fij_Al*fij_Al*(1+value[m]);

         for(unsigned i=0;i<getNumberOfAtoms();++i) setAtomsDerivatives(valueSF[m],i,fij_Al*fij_Al*sfPrime[m][i]);
         valueSF[m]->set(structureF[m]);
         setBoxDerivatives  (valueSF[m],fij_Al*fij_Al*sfVirial[m]);
