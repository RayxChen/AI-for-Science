...
class IntensityMg : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit IntensityMg(const ActionOptions&);
  ~IntensityMg();
// active methods:
...
PLUMED_REGISTER_ACTION(IntensityMg,"INTENSITYMG")

void IntensityMg::registerKeywords( Keywords& keys ){
...
IntensityMg::IntensityMg(const ActionOptions&ao):
...
IntensityMg::~IntensityMg(){
  delete nl;
}

void IntensityMg::prepare(){
  if(nl->getStride()>0){
...
void IntensityMg::calculate()
{
  ...
  for(unsigned int m=0;m<qhist;++m){
    ...
    q=4*pi*std::sin(theta[m]*pi/180)/lamda;
      ...
      fij_Mg= 5.4204*exp(-2.8275*(q/(4*pi))*(q/(4*pi)))+2.1735*exp(-79.2611*(q/(4*pi))*(q/(4*pi)))+1.2269*exp(-0.3808*(q/(4*pi))*(q/(4*pi)))+2.3073*exp(-7.1937*(q/(4*pi))*(q/(4*pi)))+0.8584;
...
//   }
       ...
       comm.Sum(sfVirial);
         ...
         structureF[m]=fij_Mg*fij_Mg*(1+value[m]);

         for(unsigned i=0;i<getNumberOfAtoms();++i) setAtomsDerivatives(valueSF[m],i,fij_Mg*fij_Mg*sfPrime[m][i]);
         valueSF[m]->set(structureF[m]);
         setBoxDerivatives  (valueSF[m],fij_Mg*fij_Mg*sfVirial[m]);
