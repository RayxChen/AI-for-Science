...
class IntensityNa : public Colvar {
  bool pbc;
  bool serial;
  NeighborList *nl;
  bool invalidateList;
...
public:
  explicit IntensityNa(const ActionOptions&);
  ~IntensityNa();
// active methods:
...
PLUMED_REGISTER_ACTION(IntensityNa,"INTENSITYNA")

void IntensityNa::registerKeywords( Keywords& keys ){
...
IntensityNa::IntensityNa(const ActionOptions&ao):
...
IntensityNa::~IntensityNa(){
  delete nl;
}

void IntensityNa::prepare(){
  if(nl->getStride()>0){
